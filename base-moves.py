...
file_open = open(file_name[0], 'rb')

kt_file = file_open.read()
...
#-----------------------------------------------------------------------------------------#
            ...
            вместо использования dict можно использовать другие структуры данных
    В других ситуациях - использовать модуль collections:
        collections.deque, collections.defaultdict, collections.OrderedDict, collections.namedtuple()
...
list_for_dict = ["self.ui.check_step2_input" + str(i) for i in range(1,65)]
dict_check = {key:val for key, val in zip([i for i in range(1,65)], list_for_dict)}
print(list_for_dict)
print(dict_check)
...
self.exit_dict_MPM["kt_mnum"] = button.text()  #  добавление в словарь по ключу

print('self.exit_dict_MPM["kt_mnum"]', self.exit_dict_MPM["kt_mnum"])

self.exit_dict_MPM.get("kt_mnum")
...
#         Генерация словаря имен нумерованных переменных, для обращения к ним по номеру 
#   final_dict = {1: self.ui.combo_diap_1, 2: self.ui.combo_diap_2, ...
        num = 256
        key_arg = [x for x in range(1, num+1)]
        val_arg = [eval(f'self.ui.combo_diap_{x}') for x in range(1, num+1)]
        final_dict = dict(zip(key_arg, val_arg))

#  Проверка наличия граничного значения margin_max в списке list_input
def check_margin(self, list_input, margin_max):
        flag = 0
        for elem in list_input:
            if elem  > margin_max:
                ...
                break
        if flag == 1:
            self.show_window(f"Неверный ввод граничных значений. Максимальное значение: {margin_max}")
        return flag
#-----------------------------------------------------------------------------------------#


'''
# цикл чтобы не повторять одинаковый код
